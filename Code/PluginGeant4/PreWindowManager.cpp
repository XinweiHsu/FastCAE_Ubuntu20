...
namespace Geant4
{
	...
	void PreWindowManager::updataTrackProperty()
	{
		QList<MeshData::MeshKernal*> klist = _typeKernalHash.values(1);
		for (auto k : klist)
		{
			vtkActor* ac = _viewProvider->getActorByKernal(k);
			if(ac== nullptr) continue;
			ac->GetProperty()->SetColor(0, 0, 1.0);
		}
		klist = _typeKernalHash.values(2);
		for (auto k : klist)
		{
			vtkActor* ac = _viewProvider->getActorByKernal(k);
			if(ac == nullptr) continue;
			ac->GetProperty()->SetColor(1.0, 0, 0);
		}
		klist = _typeKernalHash.values(3);
		for (auto k : klist)
		{
			vtkActor* ac = _viewProvider->getActorByKernal(k);
			if(ac == nullptr) continue;
			ac->GetProperty()->SetColor(0, 1.0, 0);
		}
		_preWindow->reRender();
